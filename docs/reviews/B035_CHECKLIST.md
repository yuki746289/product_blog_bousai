...
| C02 出典・安全性 | PASS | 確認済み一次・公的/メーカー情報を優先。source_checked_at=2026-09-17 |
...
## 7. 2026-09-17 説明品質・安全境界の再確認

- 警戒レベル3「高齢者等避難」を年齢だけの全国一律ルールとして扱わず、危険な場所・避難所要時間・支援者を含めて説明した。
- 個別避難計画に支援者が記載されていても到着を保証するものではないため、連絡不能時・家族不在時・介護サービス停止時の代替連絡先まで確認する構成にした。
- 指定福祉避難所は受入対象、直接避難の可否、開設・移送方法が自治体・施設で異なることを明示した。
- 常用薬はサイト独自の固定日数を示さず、薬剤情報の複線化と医師・薬剤師への事前相談を優先した。
- 説明品質トリアージはレビュー対象抽出用であり、文字数やスコアを下げるための水増しは行っていない。
