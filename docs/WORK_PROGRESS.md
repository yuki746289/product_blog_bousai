...
- done: 15都道府県とB082〜B096のID、3バッチ構成、歴史データの扱い、避難場所ポリシーを `docs/research/LINEAR_RAINBAND_PREFECTURE_PLAN_20260921.md` に定義。Batch 1のB082鹿児島・B083宮崎・B084熊本・B085長崎・B086大分について、一次情報に基づく県別年表、地形・河川等の差別化、防災・避難導線を本文へ実装。5記事すべて2,500字基準以上。preview・registry・特集ナビ・個別/共通レビュー・退行テストを追加済み。PR #120をマージし、CI run #242、deploy run #94でテスト・production build・FTPS転送・本番スモーク・サイトマップ監査までPASS。
- remaining: Batch 2（高知・和歌山・三重・静岡・千葉）、Batch 3（東京・大阪・愛知・石川・富山）の作成・検証。全15都道府県のユーザー確認。
- next_action: Batch 2（高知・和歌山・三重・静岡・千葉）を一次情報から作成し、CI・本番確認まで進める。
- blocker: なし
