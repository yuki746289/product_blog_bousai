...
| LINEAR-CASE-002 | A | 線状降水帯 全事例表の再構成・高精度化 | REOPENED | 千葉B091再監査、同一豪雨内の発生速報回数を明示、表内raw `<br>`を全件除去してCI確認 | NO |
...
| CASE-QUALITY-001 | A | 実在災害事例の全記事品質監査・最新事例補強 | REOPENED | 千葉の事例表現不足と表内raw HTML漏れを修正し、再発防止テスト後に修正概要を報告 | NO |
...
- done: 正本ルールPR #17〜#20を反映し、県別14記事・全国/地域別を再構成後、PR #126/#127とproduction run #99で本番反映まで完了。その後、B091千葉について、現行公式一覧で確認できる独立した線状降水帯豪雨2件という整理自体は維持しつつ、2026年8月豪雨内の発生速報3回と2023年9月豪雨内の記録的短時間大雨情報11回が表から読み取れない点を再監査。独立イベント数を水増しせず、同一イベント内の繰り返し発生・発表回数を別表示する修正を実施。県別Markdown表のraw `<br>`46箇所も横断除去し再発防止テストを追加済み。
- remaining: public PR作成 / CI / private正本同期 / デプロイ前報告 / ユーザー明示許可 / 許可後のみ本番反映 / 本番確認 / ユーザー確認
- next_action: 修正ブランチのCIを通し、千葉事例とraw `<br>`修正のデプロイ前概要を報告する。
- blocker: 本番デプロイはユーザー明示許可待ち
- related: `docs/reviews/B091_CHECKLIST.md`, `docs/reviews/DISASTER_CASE_AUDIT_20260921.md`, `docs/reviews/DISASTER_CASE_PREDEPLOY_REPORT_20260921.md`, PR #126, PR #127, production run #99
- updated_at: `2026-09-22`
- user_approval: `NO`
