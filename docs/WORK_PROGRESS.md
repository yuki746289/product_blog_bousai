...
- done: private正本ルールを更新し、B001〜B097の一次監査を実施。線状降水帯系は全国年表B068・地域別B073〜B077・県別14都県まで新方式の再調査を完了し、直近/過去各5〜10件の努力義務、候補母集団化、重複排除、分類分離、観測記録照合を適用。
- remaining: 線状降水帯以外で実在災害事例を複数掲載する記事の抽出 / 新方式による直近・過去事例再調査 / 5件未満理由記録 / 記事別レビュー・registry同期 / CI / デプロイ前報告 / ユーザー明示許可 / 本番反映 / 本番スモーク / ユーザー確認
- next_action: 線状降水帯以外の記事から、複数の実在災害事例を表・年表・本文で列挙している記事を抽出し、件数が薄い記事から公式アーカイブ起点で再監査する。
- blocker: 線状降水帯系は内部監査完了。サイト全体の非線状降水帯事例監査が未完了
...
- done: 正本ルールPR群を反映。B068全国年表、B073〜B077地域別5記事、B082〜B092/B094〜B096県別14都県を新調査方式で再監査。地方気象台・気象庁公式アーカイブを母集団化し、直近/過去を各5〜10件へ再構成。地域別では同一広域イベントを県ごとに重複計上せず統合。全対象で直近5〜10件・過去5〜10件を達成し、実発生/関連豪雨/予測/歴史事例を分離。raw `<br>`・literal `\n` 0件、registry・記事別レビュー・監査台帳・回帰テスト更新済み。
- remaining: ユーザー明示許可 / mainマージ・本番反映 / 本番スモーク / ユーザー確認
- next_action: デプロイ前修正概要・検証結果をユーザーへ報告し、明示許可があるまで待機する。
- blocker: 本番デプロイはユーザー明示許可待ち
- related: `docs/research/LINEAR_CASE_REAUDIT_20260922.md`, `docs/reviews/LINEAR_CASE_REAUDIT_PREDEPLOY_20260922.md`, public PR #129, private rules
- updated_at: `2026-09-22`
- user_approval: `NO`
...
- completion_criteria: 県別シリーズ全対象の公式導線 / GSI導線 / 東京23区・名古屋16区セレクター / モバイル・アクセシビリティ確認 / unit test / production build / 本番表示確認 / ユーザー確認
- done: 既存14都県に自治体公式ページ＋国土地理院の指定緊急避難場所/指定避難所データへの共通shelter finderを記事上部に追加。今回B098神奈川県にも同じ導線を追加し、県別シリーズ15都県へ拡張。東京B092は23区、愛知B094は名古屋16区を選択して各自治体公式ハザードマップを開けるUIを追加。本文側は地域固有の避難ポイントだけ残し、重複していた共通説明を削除。モバイル1列化、キーボード操作、冠水後の遠距離移動注意を実装。正本ルールPR #20をmainへ反映。記事別/共通チェックリストへC02-29〜C02-35相当の確認を記録。
...
### LINEAR-MAJOR-001 県別記事の重大被害・土砂災害横断監査＋神奈川県追加

- status: `INTERNAL_CHECK_DONE`
- priority: `A`
- approved_spec: 線状降水帯の有無や雨量だけで事例を選ばず、人的被害・大規模住家被害・土砂災害・河川氾濫/堤防決壊・都市型水害・交通/ライフライン障害等の重大被害を一次資料で確認する。同一台風・豪雨の複数被害は1イベントの内訳として整理する。既存県記事にも横断適用する。
- completion_criteria: C02-47〜C02-50適用 / 既存14都県横断監査 / 必要県の本文補強 / 神奈川県実発生確認 / B098新規作成 / registry・preview・特集ナビ・パンくず・避難所導線 / 回帰テスト / デプロイ前報告 / ユーザー明示許可 / 本番反映 / 本番スモーク
- done: 既存14都県を横断監査し、B083宮崎・B086大分・B090静岡・B091千葉・B095石川を補強。B090へ2021年熱海市伊豆山土石流、B091へ2026年台風25号の北印旛沼等の堤防決壊、B095へ2024年能登豪雨の人的・土砂・河川被害を追加。B068全国年表/B074関東甲信では台風25号の千葉・神奈川被害を同一イベントとして統合。神奈川県は2025年9月5日の県内初線状降水帯を横浜地方気象台資料で確認し、B098を新規作成。直近10件・過去7件、横浜の内水/崖崩れ、西部山地の積算雨量、2026年台風25号速報を反映。B098 registry・preview・県一覧・パンくず・避難所導線・回帰テストを追加。レジストリB001〜B098でarticle_id / public path / preview path重複0件を確認。
- remaining: PR最終確認 / デプロイ前報告 / ユーザー明示許可 / mainマージ・本番デプロイ / 公開後スモーク / 2026年台風25号速報値の短期再確認
- next_action: PRを作成し、変更概要と検証結果をユーザーへ報告する。明示許可があるまでmainへマージしない。
- blocker: 本番デプロイはユーザー明示許可待ち
- related: `docs/reviews/PREFECTURE_MAJOR_DAMAGE_AUDIT_20260924.md`, `content/articles/B098_linear_rainband_kanagawa.md`, `docs/reviews/B098_CHECKLIST.md`
