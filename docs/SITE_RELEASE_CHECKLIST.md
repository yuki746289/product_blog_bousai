# 防災ブログ サイト公開チェックリスト

更新日: 2026-09-21

記事単位のレビューとは別に、サイト全体を公開可能と判断する前に確認する。

## 1. ページ構成

- [ ] トップページが存在する
- [ ] グローバルナビのカテゴリページが存在する
- [ ] Q&Aページが存在する
- [ ] 運営方針 `about.html` が存在する
- [ ] 免責事項 `disclaimer.html` が存在する
- [ ] プライバシーポリシー `privacy.html` が存在する
- [ ] 広告について `advertising.html` が存在する
- [ ] お問い合わせ受付を公開する場合のみ `contact.html` を公開導線へ表示する
- [ ] 受付停止中は `contact.html` を `noindex` のまま保持し、フッター・ナビゲーションから非表示にする
- [ ] 記事本数だけを完成判定にしていない

## 2. リンク

- [ ] 作成済みページへの「準備中」表記が残っていない
- [ ] `href="#"` の内部リンクが残っていない
- [ ] planned用クラス・data属性が作成済みページに残っていない
- [ ] フッターの公開固定ページ4リンク（運営方針・免責・プライバシー・広告）が全ページで有効
- [ ] 内部リンク先のHTMLファイルが存在する
- [ ] 外部リンクは必要に応じて `noopener noreferrer` 等を使用している

## 3. 共通フッター・広告

- [ ] 全公開ページに共通フッターがある
- [ ] Amazonアソシエイト参加表示が共通位置にある
- [ ] 広告ページでアフィリエイト関係を説明している
- [ ] 商品購入CTAと内部商品紹介CTAを視覚的に区別している

## 4. CSS・UI役割

- [ ] 章見出しはsection role
- [ ] 要点・結論はsummary role
- [ ] 注意はwarning role
- [ ] 危険・禁止はdanger role
- [ ] 出典はinfo role
- [ ] 商品紹介記事CTAはproduct role
- [ ] Amazon購入CTAはpurchase role
- [ ] 関連記事はrelated role
- [ ] 表ヘッダーはtable role
- [ ] 同じ色を意味の異なる主要UIへ安易に流用していない
- [ ] 章見出し・重要ポイント・注意・商品CTA・Q&A CTAが、色だけでなく形・枠・アイコン・余白でも判別できる
- [ ] 色だけでなく枠・文言・余白でも役割を区別できる

## 4.1 デザイン・UX横断レビュー

共通CSS、トップ、カテゴリ、固定ページ、新機能を変更した場合に実施する。

- [ ] ページを短時間見て各領域の役割が分かる
- [ ] 記事・商品・警告・最新情報等を同じカード表現で誤認させていない
- [ ] 新しい要素だけが強い背景色・影・サイズで不自然に浮いていない
- [ ] 重要度の高い情報から自然に視線が流れる
- [ ] 同程度に強いCTAが1画面内で競合していない
- [ ] 余白・文字サイズ・罫線・角丸・影の使い方がサイト全体で調和している
- [ ] PCとスマートフォンで情報の優先順位が維持されている
- [ ] デザイン差を色だけに依存していない

## 4.2 アクセシビリティ横断レビュー

- [ ] キーボードで主要操作へ到達できる
- [ ] フォーカス表示を確認した
- [ ] リンク文言だけで遷移先が分かる
- [ ] 重要情報を色だけで区別していない
- [ ] 小さすぎる本文・注記・CTAがない
- [ ] スマートフォンでタップ対象が密集していない
- [ ] 表・開閉UI・動的UIがモバイルでも利用できる
- [ ] 安全上重要な情報が画像内文字だけになっていない

## 5. プライバシー・問い合わせ

- [ ] Google Analytics利用をプライバシーポリシーへ記載
- [ ] Amazon等の外部サービスを説明
- [ ] 実在しない連絡先を掲載していない
- [ ] 問い合わせ受付状態を正確に表示
- [ ] 問い合わせフォーム等を追加した場合、プライバシーポリシーを同時更新

## 5.5 仕様・成果物の一致確認

- [ ] タイムアウト・ツール失敗・接続切断を理由に、承認済み仕様を別仕様へ置き換えていない
- [ ] 承認済みの成果物種別（画像 / HTML図解 / 動画 / ファイル等）がそのまま実装されている
- [ ] 「画像」と「HTML/CSS図解」など、見た目や目的が近くても異なる成果物を同等扱いしていない
- [ ] 代替実装を行った場合、事前にユーザー承認を取得している
- [ ] PC・スマートフォン双方で、要求された成果物が実際に見える / 動く / 取得できることを確認した
- [ ] コードやDOMに存在するだけでなく、公開画面上の配置・順序・可視性が仕様どおりである
- [ ] ユーザー承認前は、内部チェックがPASSでも最終状態を「完了」にしていない

## 6. 公開実装

- [ ] 本番URL・ディレクトリ構造へ変換してもリンク切れしない
- [ ] 全本番ページにGoogle Analyticsタグが必要に応じて入る
- [ ] robots/noindex等が本番公開方針と一致
- [ ] PC幅で主要ページを目視確認
- [ ] スマートフォン幅で主要ページを目視確認
- [ ] 商品画像の実表示をブラウザで確認
- [ ] ホバー・フォーカス・リンク色を目視確認

## 6.1 検索エンジン向け公開ファイル

- [ ] `sitemap.xml` が存在し、公開対象HTMLを列挙している
- [ ] `sitemap.xml` に非公開の `contact.html` が含まれていない
- [ ] `robots.txt` が存在する
- [ ] `robots.txt` に本番 `sitemap.xml` のURLが記載されている
- [ ] 本番URLから `sitemap.xml` と `robots.txt` をHTTP取得できる

## 6.2 Search Console・インデックス確認

初回公開、URL構造変更、sitemap/robots変更時に実施する。

- [ ] Search ConsoleのプロパティURLが本番URLと一致している
- [ ] `sitemap.xml` が通常アクセスでHTTP 200
- [ ] `sitemap.xml` がGooglebot相当User-AgentでもHTTP 200
- [ ] Search Consoleへ `sitemap.xml` を送信した
- [ ] サイトマップの取得ステータスが成功している
- [ ] 404 / 取得失敗時は `docs/SEARCH_CONSOLE_INDEXING.md` の順序で切り分けた
- [ ] サイトマップ成功とページのインデックス登録完了を混同していない
- [ ] 必要に応じてトップ・主要pillar記事をURL検査した

## 6.3 SEOメタデータ

- [ ] 全公開HTMLに固有の `<title>` がある
- [ ] 全公開HTMLに `meta name="description"` がある
- [ ] titleの完全重複がない
- [ ] meta descriptionの完全重複がない
- [ ] title / H1 / meta descriptionが各ページの主検索意図と整合している
- [ ] 情報記事と商品記事が同一検索意図で競合していない
- [ ] 本番ビルドのSEOメタデータ自動検証がPASSしている

## 6.4 技術品質・パフォーマンス

- [ ] ページ固有CSS/JSと共通CSS/JSの責務が適切
- [ ] JavaScript構文チェックがPASS
- [ ] 主要CSS/JS/画像/JSONが本番HTTPで取得できる
- [ ] 外部API・外部画像失敗時のフォールバックがある
- [ ] 動的情報で取得失敗を「問題なし」と誤表示しない
- [ ] 画像による大きなレイアウトシフトを抑えている
- [ ] 不要な巨大画像・重複CSS/JS・第三者スクリプトを増やしていない
- [ ] Lighthouse等を使う変更では変更前後と主要ボトルネックを比較した
- [ ] 複数の更新経路が同一ファイルを意図せず上書きしない

## 6.5 マーケティング・計測（条件付き）

- [ ] ページ/機能の対象読者と目的を一文で説明できる
- [ ] 読後または利用後の次の行動が自然
- [ ] 商品・CTAが安全情報や検索意図を邪魔していない
- [ ] 新規GA4イベントは改善判断に必要なものだけ追加した
- [ ] 計測イベントへ不要な個人情報を送っていない
- [ ] 公開後に確認する主指標を決めた


## 6.5A 既存ページ変更の事前承認ゲート

既存ページの本文・表・画像・リンク・UI・ナビゲーション・共通CSS/JS等を変更する場合は、`CONTENT_CREATION_RULES.md` CR-09P に従う。

- [ ] 実装前に修正案・対象ページ・主な影響をユーザーへ提示した
- [ ] ユーザーの明示的な承認後にページ出力の変更へ進んだ
- [ ] 承認済み案から大きく変更した場合は再確認した
- [ ] 未承認のページ変更を同じPRへ混在させていない
- [ ] 「画像」と「HTML/CSS図解」等、承認された成果物種別を無断で別種へ置き換えていない

このゲートは **6.6の本番デプロイ許可とは別**である。修正実装の承認を得ても、本番デプロイには別途6.6の明示許可が必要。

## 6.6 デプロイ許可ゲート

2026-09-21以降、本番デプロイは**ユーザー明示許可制**とする。

- [ ] 修正・追加したページ、記事、ルール、コードの概要をデプロイ前にユーザーへ報告した
- [ ] 変更対象ファイルまたは対象ページ、主要な変更内容、検証結果、既知の残課題を報告した
- [ ] 本番への影響範囲（記事のみ / 共通UI / sitemap / JS / CSS 等）を報告した
- [ ] 「デプロイしてよい」旨のユーザーの明示的な許可を得た
- [ ] 許可前にproduction deploy / FTPS転送 / 本番反映を実行していない
- [ ] 許可後に修正内容が大きく変わった場合、再度修正概要を報告して許可を取り直した

**ブランチ上の調査・修正・レビュー・テスト・PR準備は許可前でも実施してよいが、本番デプロイは実行しない。**

## 7. 最終判定

- READY_TO_UPLOAD: `YES / 完了`
- READY_TO_PUBLISH: `YES`
- DEPLOYMENT_STATUS: `PUBLISHED`
- PUBLIC_URL: `https://bousaikun.ashigaru.jp/`
- 本番HTTPスモークテスト: PASS


## 構造化データ・記事日付

- [ ] 全公開記事で「公開日」「最終更新日」「情報確認日」が表示される
- [ ] `published_at` / `modified_at` / `source_checked_at` が台帳と一致する
- [ ] 全公開記事に生成済みBlogPostingが1件ある
- [ ] 全公開記事に生成済みBreadcrumbListが1件ある
- [ ] datePublished / dateModifiedが公開表示と一致する
- [ ] JSON-LDがJSONとして解釈できる
- [ ] BreadcrumbListがページ役割に応じた自然な階層数で、内部URL文字列ではなくユーザー向けの短い名称になっている
- [ ] パンくずの先頭名称・カテゴリ/特集名称・末尾名称がサイトの情報設計と一致している
- [ ] Google Rich Results Test等の外部検証が必要なリリースでは結果を記録する


## 本番配信内容の自動確認

- [ ] 本番HTTPスモークでBlogPosting / BreadcrumbListの生成済みJSON-LDを確認する
- [ ] 本番 `bousai_common.js` に `amazon_click` / `product_guide_click` が配信されていることを確認する
- [ ] Wikimedia Commonsローカル画像manifestが本番HTTP 200で取得できることを確認する
